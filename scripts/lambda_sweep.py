"""Shrinkage (λ) robustness sweep for method D (pre-registered addendum 2026-08-05b).

Same fully-PIT pipeline as scripts/cap_sweep.py, but sweeping method D's shrink
parameter λ over {0.0, 0.2, 0.4, 0.5, 0.6, 0.8, 1.0}:

    w = solve(λ·I + (1−λ)·C, max(IC, 0)) → clip → cap 0.25 → renormalize

λ=0 trusts the correlation matrix fully (raw Markowitz); λ=1 ignores it (pure
clipped-IC weights). Method A at nominal cap 0.25 is the fixed baseline. Research
only — the read is the SHAPE; the argmax λ is not adopted.

Outputs to output/crowding/incremental_weights/:
  lambda_sweep.csv / lambda_sweep_equity.csv

Usage: python scripts/lambda_sweep.py
"""
...
from scripts.incremental_weight_study import PANEL_PKL, OUT, water_fill
from scripts.cap_sweep import weights_A
...
def weights_D(ic, ir, C, lam):
    M = lam * np.eye(len(ic)) + (1 - lam) * C.values
    try:
        w = np.linalg.solve(M, ic.clip(lower=0.0).values)
    except np.linalg.LinAlgError:      # λ=0 with a singular C — pseudo-inverse
        w = np.linalg.pinv(M) @ ic.clip(lower=0.0).values
...
def main():
    ...
    books = ["A_0.25"] + [f"D_lam{l:.1f}" for l in LAMBDAS]
    ...
    for t in DERIV_POINTS:
        ...
        wv = {"A_0.25": weights_A(ic_mean, ic_ir, CAP)}
        for l in LAMBDAS:
            wv[f"D_lam{l:.1f}"] = weights_D(ic_mean, ic_ir, C, l)
    ...
    out.to_csv(OUT / "lambda_sweep.csv", index=False)
    pd.DataFrame(curves).to_csv(OUT / "lambda_sweep_equity.csv")
    with pd.option_context("display.width", 200, "display.float_format",
                           lambda v: f"{v:.3f}"):
        print(out.to_string(index=False))
    print(f"\nwrote {OUT}/lambda_sweep.csv + lambda_sweep_equity.csv")
